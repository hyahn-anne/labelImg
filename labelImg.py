...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        select = action(get_str('selectBox'), self.select_box, 'n', 'select', get_str('selectBox'))
        select_prev = action(get_str('selectBox'), self.select_prev_box, 'b', 'select', get_str('selectBox'))
        ...
        self.actions = Struct(save=save, save_format=save_format, saveAs=save_as, open=open, close=close, resetAll=reset_all, deleteImg=delete_image,
                              lineColor=color1, create=create, delete=delete, edit=edit, copy=copy, select=select,
        ...
        add_actions(self.menus.file,
                    (open, open_dir, change_save_dir, open_annotation, copy_prev_bounding, self.menus.recentFiles, save, save_format, save_as, close, reset_all, delete_image, select, select_prev, quit))
    ...
    def save_labels(self, annotation_file_path):
        ...
        try:
            if self.label_file_format == LabelFileFormat.PASCAL_VOC:
                ...
                yolo_ext = ".txt"
                yolo_file_path = annotation_file_path[:-4] + yolo_ext
                self.label_file.save_yolo_format(yolo_file_path, shapes, self.file_path, self.image_data, self.label_hist,
                                                 self.line_color.getRgb(), self.fill_color.getRgb())
            elif self.label_file_format == LabelFileFormat.YOLO:
    ...
    def select_box(self):
        ...
        print(self.start_idx, len(self.label_list))

        if self.start_idx >= len(self.label_list):
            self.start_idx = len(self.label_list) - 1
        elif self.start_idx < len(self.label_list):
            if len(self.label_list) > 0:
                item = self.current_item()
                if item == None:
                    print('select first box')
                    item = self.label_list.item(0)
                else:
                    print('select ', self.start_idx, ' box')
                    item = self.label_list.item(self.start_idx)
            self.canvas.select_shape(self.items_to_shapes[item])
