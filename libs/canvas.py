...
class Canvas(QWidget):
    ...
    def keyPressEvent(self, ev):
        ...
        if ((md & Qt.ControlModifier) == Qt.ControlModifier) or ((md & Qt.ShiftModifier) == Qt.ShiftModifier):
            md_pressed = True
            if (md & Qt.ControlModifier) == Qt.ControlModifier:
                is_ctrl = True
            else:
                is_ctrl = False
        else:
        ...
        if key == Qt.Key_Escape and self.current:
            ...
            self.finalise()
        elif key == Qt.Key_Left and self.selected_shape and (not md_pressed):
            self.move_one_pixel('Left')
        elif key == Qt.Key_Right and self.selected_shape and (not md_pressed):
            self.move_one_pixel('Right')
        elif key == Qt.Key_Up and self.selected_shape and (not md_pressed):
            self.move_one_pixel('Up')
        elif key == Qt.Key_Down and self.selected_shape and (not md_pressed):
            self.move_one_pixel('Down')
        elif key == Qt.Key_Left and self.selected_shape and md_pressed:
            if is_ctrl:
                self.resize('left', 'plus')
            else:
                self.resize('left', 'minus')
        elif key == Qt.Key_Right and self.selected_shape and md_pressed:
            if is_ctrl:
                self.resize('right', 'plus')
            else:
                self.resize('right', 'minus')
        elif key == Qt.Key_Up and self.selected_shape and md_pressed:
            if is_ctrl:
                self.resize('up', 'plus')
            else:
                self.resize('up', 'minus')
        elif key == Qt.Key_Down and self.selected_shape and md_pressed:
            if is_ctrl:
                self.resize('down', 'plus')
            else:
                self.resize('down', 'minus')
    ...
    def resize(self, direction, ctrl):
        if direction == 'left':
            if ctrl == 'plus':
                self.selected_shape.points[0] += QPointF(-1.0, 0)
                self.selected_shape.points[3] += QPointF(-1.0, 0)
